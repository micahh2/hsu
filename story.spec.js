...
describe('isWithinInterval', () => {
  it('should be within the interval when it\'s happening exactly now', () => {
    const now = 100000;
    const trigger = events[1].trigger;
  ...
  it('should be within the interval when it\'s happening exactly now + start', () => {
    const now = 100100;
    const trigger = events[1].trigger;
    expect(Story.isWithinInterval({ now, interval: trigger.interval, threshold: 1, start: 100 })).true;
  });
  ...
  it('should not be within the interval when now is later than end', () => {
    const now = 100000*4;
    const trigger = events[1].trigger;
    expect(Story.isWithinInterval({ now, interval: trigger.interval, threshold: 1, end: 100000*3 })).false;
  });
});
...
describe('isWithinArea', () => {
  it('should be when actor is in area', () => {
    ...
    const area = { x: 5, y: 5, width: 5, height: 5 };
  ...
  it('should not be when actor is beyond the area', () => {
    ...
    const area = { x: 5, y: 0, width: 5, height: 5 };
    expect(Story.isWithinArea({ actor, area })).false;
  });
});
...
describe('isTriggered', () => {
  ...
  it('should trigger when in area', () => {
    ...
    const areas = [{ x: 5, y: 5, width: 5, height: 5, id: 1 }];
    const trigger = events[2].trigger; // areaId: 1
